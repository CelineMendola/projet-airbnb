---
title: "Analyse de données sur les listings Airbnb à Paris"
author: "Céline Mendola"
output:
...
On charge nos différentes librairies et on récupère nos jeux de données. 
```{r}
library("data.table") 
library('R.utils')
library("tidyverse")
...
Chaque id correspond à une ligne unique du dataframe data_listings. 
```{r}
...
On crée à partir de data_listings un nouveau dataframe df avec les variables utiles pour nos premières analyses.
...
#### 2. Nombre de listings par arrondissement. 
...
#### 3. Quels sont les types de logements? 


Il y a une grande diversité de types de logements répertoriés dans la colonne "property_type". On va les reclasser en 10 types:

 * Le type "insolite" comprendra les logements dont le nom contient: treehouse|earth|nature|igloo|bungalow|island|barn|campsite|camper/RV
 * Le type "hotel" (contenant aussi les logements dont le room_type est "hotel room")
 * Le type "maison d'hôtes" (listings contenant Bed and Breakfast ou guesthouse)
 * Le type "guest_suite": suite dans une maison d'hôtes
 * Le type "loft"
 * Le type "copropriété"
 * Le type "maison" 
 * Le type "péniche"
 * Le type "appartement/studio " qui comprendra les logements type :apartment, chalet, dome, cabin, flat, place 
 * Le type "maison" (townhouse, house, cottage, villa ...) 

On a fait attention aux faux amis comme "chalet" ou "dome" en allant voir la description.
...
```
#### 4. Combien de listings selon le nombre de personnes ?


On enlève les lignes pour lesquelles accommodates vaut 0 et on groupe selon les valeurs possibles. 
```{r}
...
theme(axis.line.y=element_blank(),
          ...
          plot.title =element_text(hjust=0.5,face="bold",color="#666666"))
```
On observe une majorité de logements pourvant accueillir 2 ou 4 personnes.


#### 5. Prix journalier moyen par arrondissement sur les listings pour deux personnes

On convertit les prix qui sont tous en dollars.  
```{r}
...
```
On filtre sur les logements pour deux personnes et on convertit les prix en euros. On enlève les locations à 0.00$.
Finalement, on calcule le prix moyen par arrondissement.
```{r}
...
theme(legend.position="none")
```
#### 6.Carte des types de logement 

Ne pouvant pas tracer toutes les locations sur la carte, je filtre sur celles qui contiennent un maximum de données renseignées. 
```{r}
library("leaflet")
library("sp")
library('htmltools')
...
**Carte intéractive**,sélectionnez le type, zoomez et cliquez sur les points. 
```{r}
...
##### 7. Informations par arrondissement

On charge le fichier geojson donnant les informations gps de chaque arrondissements (site data.gouv) 
```{r}
library('geojson')
library('rgdal')
```
...
On réalise une jointure entre les prix moyen par arrondissement et notre SpatialPolygonsdataframe nommé paris. 
```{r}
...
On peut maintenant tracer notre carte. 
```{r}
library('RColorBrewer')
...
```
**Nombre de listings par arrondissement**:
...
##### 8. Evolution du nombre de listings 

On filtre sur les listing disponibles, et on calcule leur nombre par date. 
```{r}
